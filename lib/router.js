...
Router.map(function() {
  ...
  this.route("product", { 
    path: "/product/:_id",
    waitOn: function () {
      // no need to subscribe if it is already in the collection
      return Products.findOne(this.params._id) ? [] : Meteor.subscribe('product', this.params._id);
    },
    data: function() {
  ...
  this.route('suite', {
    path: "/suite/:suiteId",
    data: function() {
      return Suites.findOne(this.params._id) ? [] : Suites.find({suiteId: this.params.suiteId});
    },
    waitOn: function() {
